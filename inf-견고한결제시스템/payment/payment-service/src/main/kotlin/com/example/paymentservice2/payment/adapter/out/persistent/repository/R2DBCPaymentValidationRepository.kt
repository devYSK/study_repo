...
class R2DBCPaymentValidationRepository(
  ...
  override fun isValid(orderId: String, amount: Long): Mono<Boolean> {
    return databaseClient.sql(SELECT_PAYMENT_TOTAL_AMOUNT_QUERY)
      ...
      .handle { row, sink ->
        ...
        val totalAmount = row["total_amount"] as BigDecimal?

        Logger.info("good", "totalAmount: $totalAmount, amount: $amount")
        if (totalAmount != null && totalAmount.toLong() == amount) {
          sink.next(true)
        } else {
  ...
  companion object {
    const val SELECT_PAYMENT_TOTAL_AMOUNT_QUERY = """
