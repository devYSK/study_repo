#  레디스 모니터링 - redis-stat, 프로메테우스, 그라파나
...
-> 드래그해서 Find more data source plugins (그라파나url.com/connections/add-new-connection?cat=data-source)
...
* docu : https://redisgrafana.github.io/redis-datasource/overview/
...
1. http://서버URL/dashboard/import

2. import via grafana.com에서 위 패널 라이브러리 id인 12776 입력 후 Load

![image-20230904215210074](./images//image-20230904215210074.png)

3. Redis 데이터 소스 선택 후 Import
...
services:
  redis-stat:
    ...
    command:
      ...
      - '--server'
      - '49.50.174.151:6379/1234' # IP2:PORT2, IP3:PORT3 등으로 추가 가능. 뒤에 1234는 레디스 패스워드
    volumes:
...
# Prometheus + Grafana로 Redis Metric 수집
...
먼저 wget으로 단순하게 설치해서 실행하는 방법과 system화 해서 service에 등록하는 방법이 있습니다.
...
첫 번째 방법은 단순하게 `redis_exporter`를 실행하는 반면, 두 번째 방법은 `redis_exporter`를 시스템 서비스로 설치하고 관리합니다.
  ...
  - job_name: "redis-expoter1" # # 작업(job)의 이름 job_name 은 모든 scrap 내에서 고유해야한다
    ...
    static_configs: # 정적 설정을 사용하여 대상을 지정
      - targets: # 스크레이프할 대상의 주소
          #- <<REDIS-EXPORTER-HOSTNAME>>:9121          # Redis Exporter의 주소
          # - '{Your Node Exporter IP}:9121' #Redis Exporter의 주소
          - '49.50.174.151:9121' # Redis Exporter의 주소
...
http://그라파나IP:3000/dashboard/imports

다음 대시보드를 선택해서 이용하면 됩니다. 이미 잘 만들어진 것들이 있습니다.
...
Redis Exporter는 기본적으로 인증 메커니즘을 제공하지 않습니다. 그러나 보안을 높이기 위한 다양한 방법이 있습니다.
...
가장 간단하면서도 효과적인 방법은 리버스 프록시를 사용하여 Basic Authentication을 설정하는 것입니다. 